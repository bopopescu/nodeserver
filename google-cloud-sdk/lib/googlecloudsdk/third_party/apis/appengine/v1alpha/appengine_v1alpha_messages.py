...
class AppengineAppsServicesPatchRequest(_messages.Message):
  ...
  Fields:
    migrateTraffic: Set to true to gradually shift traffic to one or more
      ...
      api/reference/rest/v1alpha/apps.services.versions#inboundservicetype)
      and automatic scaling (https://cloud.google.com/appengine/docs/admin-
      api/reference/rest/v1alpha/apps.services.versions#automaticscaling). You
      must specify the shardBy (https://cloud.google.com/appengine/docs/admin-
      api/reference/rest/v1alpha/apps.services#shardby) field in the Service
...
class FirewallRule(_messages.Message):
  """A single firewall rule which is evaluated against incoming traffic and
  ...
  Fields:
    ...
    sourceRange: IP address or range, in CIDR format, of requests that this
      rule applies to. We will also accept the special string "*" to match all
      IPs equivilant to "0/0 and "::/0" together. Examples: 192.168.1.1 or
...
class GceNetwork(_messages.Message):
  """Network settings for connecting an App Version with a GCE Network

  Fields:
    hostProjectId: Optionally provided identifier of the project for the
      connected network if it is different from the current project id.
    name: Google Cloud Platform network this app version is internally
      connected to. Specify the short name, not the resource path.Defaults to
      default.
  """
...
class ResourceRecord(_messages.Message):
  ...
  Fields:
    dnsEntryFound: Whether the resource record exists in the domain's DNS
      configuration. This is determined using a basic DNS lookup. In order to
      serve an app out of a domain mapping, all of the domain mapping's
      records must exist in DNS.If false, the DNS records may have not been
      set up correctly or the new DNS changes have not yet propagated. Note
      that DNS configuration changes may take up to 24 hours to
      propagate.@OutputOnly
  ...
  dnsEntryFound = _messages.BooleanField(1)
  name = _messages.StringField(2)
  rrdata = _messages.StringField(3)
...
class Version(_messages.Message):
  ...
  Fields:
    ...
    defaultExpiration: Duration that static files should be cached by web
      ...
      api/reference/rest/v1alpha/apps.services.versions#staticfileshandler)
    ...
    errorHandlers: Custom static error pages. Limited to 10KB per page.Only
      returned in GET requests if view=FULL is set.
    gceNetwork: DONOTSUBMIT: Should this be up in the default view next to
      network? Connected GCE Network
  ...
  gceNetwork = _messages.MessageField('GceNetwork', 14)
